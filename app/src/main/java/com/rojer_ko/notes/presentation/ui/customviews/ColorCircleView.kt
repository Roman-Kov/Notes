package com.rojer_ko.notes.presentation.ui.customviews
...
import com.rojer_ko.notes.presentation.extensions.toPx
...
class ColorCircleView @JvmOverloads constructor(context: Context,
    ...
    @Dimension(unit = PX) var radius: Float = (defRadiusDp).toFloat().toPx()
    ...
    @ColorRes var strokeColorRes: Int = R.color.design_default_color_on_secondary
    ...
    @Dimension(unit = PX) var strokeWidth: Float = (defStrokeWidthDp).toFloat().toPx()
        set(value) {
            field = value
    ...
    init {
        ...
        val defRadiusPx = (defRadiusDp).toFloat().toPx()
        ...
        val defStrokeWidthPx = (defStrokeWidthDp).toFloat().toPx()
        strokeWidth = a.getDimension(
            R.styleable.ColorCircleView_strokeWidth, defStrokeWidthPx)

        strokeColorRes = a.getResourceId(
            R.styleable.ColorCircleView_strokeColor, R.color.design_default_color_on_secondary)
        a.recycle()
    }
